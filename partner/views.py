        ...
        if check_captcha.is_valid is False: # captcha not valid
            ...
            if is_valid_email(email):
                ...
                if len(users) > 0:
                    ...
                    body = _("This email is generated automatically from %(site)s\n\nUsername: %(username)s\nPassword: %(password)s\n\n%(live_url)s\n\nPlease, do not answer this email") % {'site':SITE_TITLE,'username':user.username,'password':key,'live_url':LIVE_URL}
                    email = EmailMessage(subject, body, to=[user.email])
                    email.send()
                    email = ''
                    msg = _('A new password is  sent to %(email)s') % {'email':user.email}
                    message.append(msg)
                else:
                    msg = _('Sorry. This email do not exist. Try again')
                    message.append(msg)
            else:
                msg = _('Sorry. This email is not valid. Try again')
                message.append(msg)
        ...
        if data['password1'] == data['password2']:
            if len(data['password1']) >= KEY_LENGHT:
                ...
                if request.user.email:
                    ...
                    body = _("This email is generated automatically from %(site)s\n\nNew password: %(password)s\n\n%(live_url)s\n\nPlease, do not answer this email") % {'site':SITE_TITLE,'password':data['password1'],'live_url':LIVE_URL}
                    email = EmailMessage(subject, body, to=[request.user.email])
                    email.send()
                error = _("New password is added")
            else:
                error = _("Sorry. The password needs  %(size)s characters or more. Try again") % {'size':KEY_LENGHT}
        else:
            error = _("Sorry. Password do not match. Try again")
